...
private:
    /* Reads a specific friend's off-LibraryMixer shares from disk. */
    OffLMShareItem* internalReadFriendOffLMShares(unsigned int friend_id, const QString &xmlHash) const;
    ...
    /* This meta data is used to keep the friends' XMLs up to date. */
    struct friendXmlInfo {
        friendXmlInfo() {};
        friendXmlInfo(const QString &xml_hash, qlonglong xml_size) :xml_hash(xml_hash), xml_size(xml_size) {}

        QString xml_hash;
        qlonglong xml_size;
    };
    QHash<int, friendXmlInfo> friendXmlInfos;
