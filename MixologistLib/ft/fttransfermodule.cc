...
/* Start by requesting chunks of 5k. In practice this works out to a little over 1KB/s */
const double FT_TM_FAST_START_RATE = 5120;
const uint32_t FT_TM_MAX_RESETS  = 5;

const uint32_t FT_TM_MINIMUM_CHUNK = 128; /* ie 1/8Kb / sec */
const uint32_t FT_TM_RESTART_DOWNLOAD = 5; /* 5 seconds */
...
const int32_t FT_TM_FAST_RTT    = 1;
//const int32_t FT_TM_STD_RTT     = 5;
const int32_t FT_TM_SLOW_RTT    = 9;

peerInfo::peerInfo(int _librarymixer_id)
    :librarymixer_id(_librarymixer_id),state(PQIPEER_NOT_ONLINE),actualRate(0),
     offset(0),chunkSize(0),receivedSize(0),lastTS(0),recvTS(0), lastTransfers(0), nResets(0),
     rtt(0), rttActive(false), rttStart(0), rttOffset(0),mRateIncrease(1), fastStart(true) {
    cert_id = peers->findCertByLibraryMixerId(_librarymixer_id);
    return;
}
...
ftTransferModule::ftTransferModule(ftFileCreator *fc, ftDataMultiplex *dm, ftController *c)
    ...
    MixStackMutex stack(tfMtx); /******* STACK LOCKED ******/

    mSize = mFileCreator->getFileSize();
    mFileStatus.hash = mFileCreator->getHash();
    ...
    if (mFileCreator->finished()) mFlag = 1;
...
bool ftTransferModule::setFileSources(QList<int> sourceIds) {
    ...
    log(LOG_DEBUG_BASIC, FTTRANSFERMODULEZONE, "Resetting sources list for file " + QString(hash().c_str()));

    for (int i = 0; i < sourceIds.size(); i++) {
        peerInfo pInfo(sourceIds[i]);
        mFileSources.insert(std::pair<std::string,peerInfo>(pInfo.cert_id,pInfo));
    }
...
bool ftTransferModule::getFileSources(QList<int> &sourceIds) {
    ...
    for (it = mFileSources.begin(); it != mFileSources.end(); it++) {
        sourceIds.push_back(it->second.librarymixer_id);
    }
    return true;
}
...
bool ftTransferModule::addFileSource(int librarymixer_id) {
    ...
    for (mit = mFileSources.begin(); mit != mFileSources.end(); mit++) {
        if (mit->second.librarymixer_id == librarymixer_id) break;
    }
...
bool ftTransferModule::setPeerState(int librarymixer_id,uint32_t state) {
    ...
    if (state == PQIPEER_NOT_ONLINE) mit->second.fastStart = true;
    ...
    if (new_cert.compare(mit->second.cert_id) != 0) {
        std::map<std::string,peerInfo>::iterator newit;
        newit = mFileSources.insert(std::pair<std::string,peerInfo>(new_cert,mit->second)).first;
        newit->second.cert_id = new_cert;
...
void ftTransferModule::requestData(std::string peerId, uint64_t offset, uint32_t chunk_size) {
    ...
    std::cerr << " peerId: " << peerId;
    std::cerr << " hash: " << mFileCreator->getHash();
    std::cerr << " size: " << mSize;
    std::cerr << " offset: " << offset;
    ...
    mMultiplexor->sendDataRequest(peerId, mFileCreator->getHash(), mSize, offset,chunk_size);
}
...
#endif
    if (mFtController)
        mFtController->FlagFileComplete(mFileCreator->getHash());
    return true;
}
...
int ftTransferModule::tick() {
#ifdef FT_DEBUG
    {
        ...
        std::cerr << " mFileCreator->getHash(): " << mFileCreator->getHash();
...
bool ftTransferModule::locked_tickPeerTransfer(peerInfo &info) {
    ...
    uint32_t next_req;
    if (info.fastStart) {
        next_req = FT_TM_FAST_START_RATE;
        info.fastStart = false;
    } else next_req = info.actualRate * (1.0 + info.mRateIncrease);
    ...
    if (getChunk(req_offset,next_req)) {
        if (next_req > 0) {
            info.state = PQIPEER_DOWNLOADING;
            requestData(info.cert_id,req_offset,next_req);
