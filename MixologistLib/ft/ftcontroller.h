 ...
 * inherits MixThread (to control transfers)
 *
 */
...
public:
    ...
    ftFileControl() :mTransfer(NULL), mCreator(NULL), mState(DOWNLOADING), mSize(0), mFlags(0) {
        return;
    }
    ftFileControl(uint64_t size, std::string hash, uint32_t flags)
        :mState(DOWNLOADING), mHash(hash), mSize(size), mFlags(flags) {
        return;
    }
...
/* Used to store things in ftController's mPendingRequests list
...
public:
    ftPendingRequest(QString _librarymixer_name, int _orig_item_id, QString fname, std::string hash,
        ...
        : librarymixer_name(_librarymixer_name), orig_item_id(_orig_item_id), filename(fname),
          mHash(hash), mSize(size), mFlags(flags), sourceIds(_sourceIds) {
        return;
    }
...
public:
    ...
    /* Setup */
    ftController(ftDataMultiplex *dm);
    //Setup function
    void    setFtSearchNItem(ftSearch *, ftItemList *);
    //ftController does not run transfers until this is called
    bool    activate();
    ...
    /* Directory Handling */
    /* Changes the download destination directory to path, saving changes to settings file.
       If trySavedSettings is true, then path is a fallback,
       and will first try to load it from the settings file instead. */
    bool    setDownloadDirectory(QString path, bool trySavedSettings = false);
    /* Changes the partials destination directory to path, saving changes to settings file.
       If trySavedSettings is true, then path is a fallback,
       and will first try to load it from the settings file instead. */
    bool    setPartialsDirectory(QString path, bool trySavedSettings = false);
    QString getDownloadDirectory();
    QString getPartialsDirectory();
...
public:
    //Automatically informs all the transfers about changes in friends' online statuses
...
private:
    ...
    /* Pulls a file off mPendingRequests and calls FileRequest on it. */
    ...
    bool mFtActive;
    //Starts out true, and becomes false when all of mPendingRequests is called
    bool mFtPendingDone;
