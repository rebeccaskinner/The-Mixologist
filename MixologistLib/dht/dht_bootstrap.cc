...
public:
    ...
    virtual void    peerStatus(std::string,
    ...
    void    stunPeer(std::string id, struct sockaddr_in peeraddr) {
        std::cerr << "stunPeer: 0x" << Util::BinToHex(id);
    ...
    virtual void    peerConnectRequest(std::string,
    ...
    virtual void    stunStatus(std::string id, struct sockaddr_in raddr, uint32_t type, uint32_t) {
        addPeer(id);
        {
            ...
            it = peerMap.find(id);
            if (it == peerMap.end()) {
                std::cerr << "peerStatus() for unknown Peer id: 0x" << Util::BinToHex(id);
                std::cerr << std::endl;
                return;
            }
    ...
    virtual void    stunSuccess(std::string id, struct sockaddr_in, struct sockaddr_in) {
        {
            ...
            it = peerMap.find(id);
            if (it == peerMap.end()) {
                std::cerr << "stunSuccess() for unknown Peer id: 0x" << Util::BinToHex(id);
                std::cerr << std::endl;
                return;
            }
            std::cerr << "stunSuccess() for id: 0x" << Util::BinToHex(id);
            std::cerr << std::endl;
