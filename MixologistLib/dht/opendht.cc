...
const std::string openDHT_Client = "Mixologist V0.4";
const std::string openDHT_Agent  = "MIX-HTTP-V0.4";
...
bool OpenDHTClient::checkServerFile(QString filename) {
#ifdef  OPENDHT_DEBUG
...
bool OpenDHTClient::loadServers(QString filename) {
    /* open the file */
    std::ifstream file(filename.toStdString().c_str());
...
bool OpenDHTClient::loadServersFromWeb(QString storename) {
    ...
    if (loadServers(iss)) {
        ...
        std::ofstream ofstr(storename.toStdString().c_str());
        ofstr << content;
        ofstr.close();
...
bool OpenDHTClient::publishKey(std::string key, std::string value, uint32_t ttl) {
    /* create request */
#ifdef  OPENDHT_DEBUG
    std::cerr << "OpenDHTClient::openDHT_publishKey() key: 0x" << Util::BinToHex(key) << " value: 0x" << Util::BinToHex(value);
    std::cerr << std::endl;
#endif
...
bool OpenDHTClient::searchKey(std::string key, std::list<std::string> &values) {
#ifdef  OPENDHT_DEBUG
    std::cerr << "OpenDHTClient::openDHT_searchKey() key: 0x" << Util::BinToHex(key);
    std::cerr << std::endl;
#endif
    ...
    while (1) {
        ...
        if (value.length() > 0) {
            ...
            std::cerr << "openDHT_searchKey() Result: 0x" << Util::BinToHex(result) << ":END:" << std::endl;
#endif
        }
