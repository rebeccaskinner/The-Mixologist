...
LibraryMixerItem::LibraryMixerItem(QDomNode &domNode)
    : linkedDomNode(domNode) {}

LibraryMixerItem::~LibraryMixerItem() {
    linkedDomNode.parentNode().removeChild(linkedDomNode);
}

QString LibraryMixerItem::title() const {
    return linkedDomNode.firstChildElement("title").text();
}

bool LibraryMixerItem::title(QString newTitle) {
    return modifyAttribute("title", newTitle);
}

unsigned int LibraryMixerItem::id() const {
    return linkedDomNode.firstChildElement("id").text().toInt();
}

bool LibraryMixerItem::id(unsigned int newId) {
    return modifyAttribute("id", QString::number(newId));
}

LibraryMixerItem::ItemState LibraryMixerItem::itemState() const {
    return (LibraryMixerItem::ItemState)linkedDomNode.firstChildElement("itemstate").text().toInt();
}

bool LibraryMixerItem::itemState(LibraryMixerItem::ItemState newState) {
    if (newState != MATCHED_TO_LENT) {
        modifyAttribute("lentto", "");
    }
    if (newState != MATCHED_TO_FILE &&
        newState != MATCH_NOT_FOUND &&
        newState != MATCHED_TO_LEND &&
        newState != MATCHED_TO_LENT) {
        clearFiles();
    }
    if (newState != MATCHED_TO_MESSAGE){
...
unsigned int LibraryMixerItem::lentTo() const {
    return linkedDomNode.firstChildElement("lentto").text().toInt();
}

bool LibraryMixerItem::lentTo(unsigned int friend_id) {
    if (itemState() != MATCHED_TO_LEND) return false;

    itemState(MATCHED_TO_LENT);
...
QString LibraryMixerItem::message() const {
    return linkedDomNode.firstChildElement("autoresponse").firstChildElement("message").text();
}

bool LibraryMixerItem::message(QString newMessage) {
    itemState(MATCHED_TO_MESSAGE);
...
QStringList LibraryMixerItem::paths() const {
    QStringList list;
...
QList<qlonglong> LibraryMixerItem::filesizes() const {
...
QStringList LibraryMixerItem::hashes() const {
    QStringList list;
    QDomElement currentElement = linkedDomNode.firstChildElement("autoresponse").firstChildElement("file");
    while (!currentElement.isNull()){
...
QList<unsigned int> LibraryMixerItem::modified() const {
...
int LibraryMixerItem::fileCount() const {
    return paths().count();
}

bool LibraryMixerItem::paths(QStringList newPaths) {
...
bool LibraryMixerItem::setFileInfo(const QString &path, qlonglong size, unsigned int modified, const QString &hash) {
...
bool LibraryMixerItem::fullyHashed() const {
    return !hashes().contains("");
}

bool LibraryMixerItem::modifyAttribute(QString attribute, QString newValue) {
...
void LibraryMixerItem::clearFiles() {
    QDomElement currentFile = linkedDomNode.firstChildElement("autoresponse").firstChildElement("file");
    while(!currentFile.isNull()) {
        if (!currentFile.firstChildElement("hash").isNull())
            emit fileNoLongerAvailable(currentFile.firstChildElement("hash").text(), currentFile.firstChildElement("size").text().toLongLong());
