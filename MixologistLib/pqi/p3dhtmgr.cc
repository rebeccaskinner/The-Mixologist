...
p3DhtMgr::p3DhtMgr(std::string id, pqiConnectCb *cb)
    ...
    ownEntry.hash1 = Util::HashId(id, false);
    ownEntry.hash2 = Util::HashId(id, true);
...
bool p3DhtMgr::findPeer(std::string id) {
    ...
    /* fill in hashes */
    ent.hash1 = Util::HashId(id, false);
    ent.hash2 = Util::HashId(id, true);
...
bool p3DhtMgr::notifyPeer(std::string id) {
    ...
    it->second.notifyPending = CONNECT_ACTIVE;
    ...
    if (peer.state >= DHT_PEER_ADDR_KNOWN) {
        ...
        if ((peer.state < DHT_PEER_PUBLISHED) ||
...
#ifdef DHT_DEBUG
            std::cerr << "PUBLISH: ";
            std::cerr << " hash1: " << Util::BinToHex(peer.hash1);
...

            {
                ...
                out << " hash1: " << Util::BinToHex(peer.hash1);
        ...
        //if ((peer.state == DHT_PEER_PUBLISHED) &&
        //  (!(peer.type & NET_CONN_TCP_EXTERNAL)))
        if (peer.state == DHT_PEER_PUBLISHED) {
            ...
            if (peer.state != DHT_PEER_PUBLISHED) {
                ...
                std::cerr << std::endl;
#endif
            } else if (peer.type & NET_CONN_TCP_EXTERNAL) {
#ifdef DHT_DEBUG
...
int p3DhtMgr::checkNotifyDHT() {
    ...
    mConnCb->peerConnectRequest(peer.id, peer.laddr, CB_DHT);
#else
    mConnCb->peerConnectRequest(peer.id, peer.raddr, CB_DHT);
#endif
...
#ifdef DHT_DEBUG
    std::cerr << "p3DhtMgr::doStun() STUN: " << Util::BinToHex(activeStunId);
    std::cerr << std::endl;
#endif
...
bool p3DhtMgr::getDhtBootstrapList() {
    ...
    std::cerr << Util::BinToHex(bootId) << std::endl;
#endif
...
std::string p3DhtMgr::BootstrapId(uint32_t bin) {
    ...
    std::string bootId = Util::HashId(genId.str(), false);
...
bool p3DhtMgr::dhtPublish(std::string idhash,
    ...
    std::cerr << "PUBLISHing: idhash: " << Util::BinToHex(idhash);
    ...
    ******/
    ...
    std::cerr << "PUBLISH: key: " << Util::BinToHex(idhash);
...
bool p3DhtMgr::dhtBootstrap(std::string storehash, std::string ownIdHash, std::string) {
#ifdef DHT_DEBUG
...
#ifdef DHT_DEBUG
    std::cerr << "p3DhtMgr::resultDHT() key: 0x" << Util::BinToHex(key);
    std::cerr << " value: 0x" << Util::BinToHex(value) << std::endl;
#endif
...
#ifdef DHT_DEBUG
            std::cerr << "p3DhtMgr::resultDHT() NOTIFY msg HASH:-> 0x" << Util::BinToHex(notifyHash) << "<-" << std::endl;
#endif
            /* call out */
...
#ifdef DHT_DEBUG
            std::cerr << "p3DhtMgr::resultDHT() BOOTSTRAP msg, IdHash:->" << Util::BinToHex(bootId) << "<-" << std::endl;
#endif
            /* call out */
...
bool p3DhtMgr::dhtResultBootstrap(std::string idhash) {
    ...
    std::cerr << Util::BinToHex(idhash) << std::endl;
#endif
...
bool p3DhtMgr::dhtResultNotify(std::string idhash) {
    ...
    if ((it != peers.end()) && (it->second.state != DHT_PEER_OFF)) {
        ...
        it->second.notifyTS      = now;
        it->second.notifyPending = CONNECT_PASSIVE;
        mDhtModifications = true; /* no wait! */
...
bool p3DhtMgr::dhtResultSearch(std::string idhash,
    ...
    if ((it != peers.end()) && (it->second.state != DHT_PEER_OFF)) {
        ...
        /* do Stun always */
        doStun = true;
        stunFlags = STUN_FRIEND | STUN_ONLINE;
    } else {
#ifdef DHT_DEBUG
...
void printDhtPeerEntry(dhtPeerEntry *ent, std::ostream &out) {
    ...
    out << " type: " << ent->type;
    out << std::endl;
    out << " hash1: " << Util::BinToHex(ent->hash1);
    out << std::endl;
    out << " hash2: " << Util::BinToHex(ent->hash2);
    out << std::endl;
    return;
}
