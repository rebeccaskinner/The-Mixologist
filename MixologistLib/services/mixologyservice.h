...
class MixologyService: public p3Service {
public:
    MixologyService();
    virtual int tick();
    /*Adds a request to mPending for an item identified by item_id from friend with librarymixer_id
      labeling the request name */
    bool LibraryMixerRequest(int librarymixer_id, int item_id, QString name);
    /*Removes a request from mPending*/
    bool LibraryMixerRequestCancel(int item_id);
    ...
    void LibraryMixerBorrowed(unsigned int librarymixer_id, int item_id);
    /*Called on completion of a borrow return download to let the borrower know the transfer is complete.*/
    void LibraryMixerBorrowReturned(unsigned int librarymixer_id, int item_id);
...
private:
    ...
    void prepFileResponse(int item_id, int status, MixologyResponse *response);
    ...
    MixMutex mMixologyServiceMutex;
};
