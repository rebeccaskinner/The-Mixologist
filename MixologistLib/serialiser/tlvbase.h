...
/* For QT */
const uint16_t TLV_TYPE_QSTR_MSG      = 0x0090;
...
bool     SetTlvQString(void *data, uint32_t size, uint32_t *offset, uint16_t type, QString out);
bool     GetTlvQString(void *data, uint32_t size, uint32_t *offset, uint16_t type, QString &in);
